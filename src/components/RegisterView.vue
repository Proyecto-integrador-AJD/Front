<template>
    <div class="register-container">
      ...
      <form @submit.prevent="handleRegister">
        ...
        <div class="input-group">
          ...
          <input id="prefix" v-model="prefix" type="integer" placeholder="Prefijo" required />
        </div>
  
        <div class="input-group">
          <label for="phone">Teléfono</label>
          <input id="phone" v-model="phone" type="integer" placeholder="Teléfono" required />
        </div>
  ...
  import MultiSelect from "primevue/multiselect"; // Importando MultiSelect
  
  export default {
    components: {
      MultiSelect, // Registrando el componente MultiSelect
    },
    setup() {
      const name = ref("");
      const lastName = ref("");
      ...
      const email = ref("");
      const password = ref("");
      const selectedLanguages = ref([]); // Variable para los idiomas seleccionados
      const loading = ref(false);
      const error = ref(null);
      const router = useRouter();
  
      // Opciones de idiomas para el MultiSelect 
  ...
  form {
    ...
    gap: 1rem;
    padding-bottom: 2rem; /* Agregado padding al fondo del formulario */
  }
